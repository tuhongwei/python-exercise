...
#获取页面内容提取
    def LoadPageContent(self,page):
        ...
        #正则获取
        pattern_link=re.compile(r'<p.*?class="top">.*?'
        ...
        for item in items:
        #头像，个人详情，名字，年龄，地区
            print(u'发现一位MM 名字叫%s 年龄%s 坐标%s'%(item[1],item[2],item[3]))
            print(u'%s的个人主页是 %s'%(item[1],item[0]))
            print(u'继续获取详情页面数据...')
        #详情页面
            detailPage=item[0]
            name=item[1]
...
#获取所有图片
    ...
    #只获取第一个相册
        photos=self.driver.find_element_by_xpath('//div[@class="mm-photo-cell-middle"]//p/a')
    ...
    def saveBrief(self,content,dir,name,speed_time):
        ...
        f.write(bytes.decode(content.encode('utf-8')))
